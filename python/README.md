...
Add to `.vscode/mcp.json` in your workspace (or your user `settings.json` under `"mcp.servers"`):
...
Then open the Command Palette (`Ctrl+Shift+P` / `Cmd+Shift+P`) and run **MCP: List Servers** to confirm it is connected.
...
```bash
pip install -e ".[dev]"
python -m mcp_cloudwatch   # run directly
```
